...
from test.conftest import TEST_WORK_DIR_RELATIVE_PATH, AUDIO_FILE_EXTENSION
...
def test_download_audio_from_youtube_from_valid_url():
    ...
    expected_download_audio_from_youtube_filepath = operating_system_proof_path(path.abspath(path.join(
        TEST_WORK_DIR_RELATIVE_PATH,
        FIRST_YOUTUBE_VIDEO_TITLE + AUDIO_FILE_EXTENSION.value
    )))
    tested_audio_filepath = youtube_download.download_audio_from_youtube(tested_url, TEST_WORK_DIR_RELATIVE_PATH, AUDIO_FILE_EXTENSION)
...
def test_fails_download_audio_from_youtube_raise_cannot_download_error_exception_with_wrong_url():
    ...
    with pytest.raises(cannot_download_error.CannotDownloadError):
        youtube_download.download_audio_from_youtube("https://youtube.com/invalid_url", TEST_WORK_DIR_RELATIVE_PATH, AUDIO_FILE_EXTENSION)
...
def test_fails_download_audio_from_youtube_raise_cannot_download_error_exception_is_a_livestream():
    ...
    with pytest.raises(cannot_download_error.CannotDownloadError):
        youtube_download.download_audio_from_youtube(lofi_girl_livestream_url, TEST_WORK_DIR_RELATIVE_PATH, AUDIO_FILE_EXTENSION)

...
def _choose_youtube_video_interactively(video_metadatas: List[YouTubeVideoMetadata]) -> YouTubeVideoMetadata|None:
    """
    ...
    for youtube_metadata in video_metadatas:
        print("Would you like to download the following YouTube video's audio ? (type 'yes' to confirm or anything else otherwise):")
        ...
        print(prompted_video_proposition)
        user_input = input()
...
def load(audio_name: str, file_extension: FileExtension, profile: str) -> Audio:
    ...
    if _is_file_loaded(audio_name + file_extension.value, profile):
        ...
        chosen_youtube_video: YouTubeVideoMetadata = _choose_youtube_video_interactively(youtube_videos_metadatas)
        youtube_video_url: str = chosen_youtube_video.youtube_url
        try:
...
def iterate_over_loading_playlist(playlist_file_absolute_path: str, playlist_profile: str) -> list[Audio]:
    ...
    @yield: Audio
    """
    playlist_lines: list[str] = parse_plain_text_playlist_file(playlist_file_absolute_path)

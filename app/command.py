''' App's commands enumeration module '''
...
class Command(Enum):
    ...
    def help(self):
        ...
        command_help = {
            ...
            Command.IMPORT: f"load all the playlist's musics by default. Alternatively load a \
            ...
            Command.PLAY: "play first audio by default . If prefixed by the playlist's audio index \
             ...
             prefixed with at least 2 arguments as audio's name, it will try to play the matching \
             audio: \"play <first_audio_name_part> <second_audio_name_part>  \
             [<other_audio_name_part>]*\"",
            ...
            Command.MODE: "show the current play mode, or with argument(s), set the play mode  \
             accordingly: {}".format(
                [str(mode) for mode in PlayMode]
            ),
            Command.VOLUME: "show current volume. Alternatively, increase or decrease the volume \
             by a percentage: \"volume up/down <percentage>\"",
            Command.LYRIC: "show the lyrics of the played song, or if prefixed by 'on' or 'off' \
             turn on or off accordingly the lyrics display.",
            Command.MOVE: "move the play moment of the current audio (in second): \"move 10.0\" : \
             move the time of the audio to the 10th second.",
            Command.PROFILE: "change the profile, or without arguments show the current profile : \
             \"profile default\": change the profile to the \"default-profile\" from the \
             configuration file",

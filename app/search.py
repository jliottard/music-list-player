''' Utility module for searching string among strings '''
...
def match_string_among_strings(searched_string: str, strings: List[str]) -> int:
    ...
    @raise raise a CannotFindAMatchError if there is no match, or the searched_string is empty
    '''
    if searched_string == "":
        if not "" in strings:
...
def match_some_strings_among_strings(
        searched_arguments: List[str], expected_arguments: List[str]
    ) -> int:

...
def _produce_audio(queue: Queue, playlist_path: str, profile: Profile, meta_query: AudioMetadata, user_interface: Interface):
    ...
    @param profile Profile: playlist's profile from the configuration file
...
def _load_playlist_in_background(playlist_path: str, profile: Profile, meta_query: AudioMetadata, user_interface: Interface) -> AudioPlayer:
    ...
    Thread(target=_produce_audio, args=(loaded_audio_queue, playlist_path, profile, meta_query, user_interface), daemon=True).start()
